...
class Config:
    ...
    VIDEO_ROOT = "/kaggle/input/ml-hack-dataset/ieee-ml-hack-data/archive/train"
    ...
    # HYPERPARAMETERS
    SEQ_LENGTH = 20        
    IMG_SIZE = 224         # EfficientNet-B0 expects 224x224. (If using B4, change to 380)
    BATCH_SIZE = 32        
    EPOCHS = 10
    LR = 1e-4
    ...
    for folder_name, label in classes.items():
        ...
        if os.path.isdir(folder_path):
            ...
            for vid_path in videos:
                ...
                # Using filename (without extension) as ID
...
def preprocess_videos(df):
    """Extracts faces using CPU to avoid MPS Adaptive Pool bug (irrelevant on CUDA, but safe)."""
    ...
    # Using Config.DEVICE here since you are likely on NVIDIA GPU (CUDA) now
    # If this crashes on Mac, change Config.DEVICE to 'cpu' inside this line
...
# ==========================================
# 5. MODEL ARCHITECTURE (EFFICIENTNET)
# ==========================================
class EfficientNetLSTM(nn.Module):
    def __init__(self, hidden_size=256, num_layers=2):
        ...
        # Use 'DEFAULT' to get the best available weights
        weights = models.EfficientNet_B0_Weights.DEFAULT 
        backbone = models.efficientnet_b0(weights=weights)
        
        # EfficientNet in torchvision is split into .features (conv layers) and .avgpool
        ...
        # (If you switch to B4 later, this needs to be 1792)
        input_size = 1280
    ...
    def forward(self, x):
        ...
        # Flatten time dimension for CNN: (batch*seq, c, h, w)
        c_in = x.view(batch_size * seq_len, c, h, w)
        
        # Pass through EfficientNet Features
        x = self.features(c_in)     # Output: (batch*seq, 1280, 7, 7)
        x = self.avgpool(x)         # Output: (batch*seq, 1280, 1, 1)
        x = torch.flatten(x, 1)     # Output: (batch*seq, 1280)
        
        # Restore time dimension for LSTM: (batch, seq, feature_size)
        lstm_in = x.view(batch_size, seq_len, -1)
        
        lstm_out, _ = self.lstm(lstm_in)
        last_out = lstm_out[:, -1, :] # Take last output of the sequence
...
def train_model():
    ...
    # Model Setup
    print(f"🏗️  Building EfficientNet-LSTM Model on {Config.DEVICE}...")
    model = EfficientNetLSTM().to(Config.DEVICE) # <--- UPDATED CLASS HERE
    ...
    torch.save(model.state_dict(), "efficientnet_b0_detector.pth")
    print("✅ Model saved as efficientnet_b0_detector.pth")
